...
<script>
	...
	map.onclick=function(e){
		for(var k =0; k<elements.length;k++){
			...
			//console.log([e.clientX-map.offsetLeft+map.width/2,e.clientY-map.offsetTop+map.height/2],elements[k]);

			if(distance([e.clientX-map.offsetLeft+map.width/2,e.clientY-map.offsetTop+map.height/2],elements[k])<25){
...
<?php 
  ...
  while ($loc = mysqli_fetch_array( $result ) )
  {
	...
	echo "<div style='display:none;' id='".$loc['name']."'>".json_encode($loc).'@'.json_encode($society['flag'])."</div>";
